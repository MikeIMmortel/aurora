...
function Row({ name, value, unit, highlight }: RowProps) {
  return (
    <div className="flex items-center justify-between py-1.5">
      ...
      <span className={`font-medium text-sm tabular-nums ${highlight ? 'text-aurora-gold' : 'text-ink'}`}>
        {value}
        <span className="text-ink-3 ml-1 text-xs">{unit}</span>
      </span>
    </div>
  );
}
  ...
  return (
    <div className="rounded-2xl border border-aurora-border bg-aurora-surface p-4">
      ...
      <div className="space-y-0.5">
        ...
        <div className="border-t border-aurora-border my-2" />
